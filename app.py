...
Navigate through the tabs below to predict varieties, view model performance, or analyze feature importance.
""")
...
# Create Tabs
tab_predict, tab_performance, tab_insights = st.tabs(["Predict", "Performance", "Insights"])
...
with tab_performance:
    ...
    st.markdown("Evaluating the model on the test dataset...")
    ...
    report = classification_report(y_test, y_pred, target_names=["Crop Variety A", "Crop Variety B", "Crop Variety C"])
...
with tab_insights:
    ...
    st.markdown("Understanding which features are most important for the model's predictions.")
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        st.bar_chart(data=dict(zip(feature_names, importances)))
    else:
